...
func (h *campaignHandler) CreateImage(c *gin.Context) {
	...
	existing, err := h.campaignService.GetCampaignByID(campaign.GetCampaignDetailInput{ID: id})
	if err != nil {
		c.HTML(http.StatusInternalServerError, "error.html", nil)
		return
	}
	...
	createCampaignInput := campaign.CreateCampaignImageInput{}
	createCampaignInput.CampaignID = id
	createCampaignInput.IsPrimary = true
	...
	createCampaignInput.User = userCampaign

	_, err = h.campaignService.SaveCampaignImage(createCampaignInput, path)
	if err != nil {
		c.HTML(http.StatusInternalServerError, "error.html", nil)
		return
	}
